...
demo.state8 = function(){}; //state1 property to our game object that's a function 
demo.state8.prototype = {
	...
	create: function(){ //setting initial values for everything
        ...
        if (statesVisited.includes(currentState) == false){ //Spawn monsters if and only if the player has not been to this state before.
            ...
            game.time.events.add(Phaser.Timer.SECOND * 25, timerEvent, this);
            game.time.events.add(Phaser.Timer.SECOND * 26, timerEvent, this);
            game.time.events.add(Phaser.Timer.SECOND * 27, timerEvent, this);
            game.time.events.add(Phaser.Timer.SECOND * 28, timerEvent, this);
...
function addItemsState8(){ //items that are added to be picked up in this state. There's a lot of "extra" code here, but new items can be added alongside the "add_item" function call under the if statement.
    
    if (statesVisited.includes(currentState) == false){
        add_item('healing crystal', 600, 450, 1);
